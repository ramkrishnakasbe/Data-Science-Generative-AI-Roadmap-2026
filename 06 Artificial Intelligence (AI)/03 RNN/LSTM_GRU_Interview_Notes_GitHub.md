...
> **GitHub rendering note:** All mathematical expressions in this document use GitHub-compatible `$...$` and `$$...$$` delimiters. GitHub renders LaTeX-formatted math in Markdown using MathJax. citeturn0search0
...
- $h_T$ = hidden state at the final time step
- $\frac{\partial L}{\partial h_t}$ = gradient of loss with respect to the hidden state at time $t$
- $\prod$ = product of the derivatives across the intermediate time steps
...
$$
g \leftarrow g \frac{\text{threshold}}{\lVert g\rVert}
$$
